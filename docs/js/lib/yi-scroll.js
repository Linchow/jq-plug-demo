...
$.fn.yi_scroll = function(options)
{
	var scroll_config={
		...
		/*"barColor" : "rgba(134,189,249,0.3)",*/
		"barColor" : "#ddd",
		"barMDColor" : "rgba(134,189,249,0.5)",
	...
	return this.each(function(){
		...
		elem.on("mouseenter",function(){
			...
			fnContentResize();
			elem.parents('.box').attr('data-isScroll',false)
		})
		elem.on("mouseleave",function(){			
			if(!opts.isBar)
			jqScrollBar.css({"opacity":0});
			w.MouseInScroll = false;
			elem.parents('.box').attr('data-isScroll',true)
		});
		...
		elem.on(sMouseWheel,function(ev){
			//console.log(ev);
			//ev.preventDefault();	
			if(elem.attr('data-isScroll')=='true'){
				ev = ev.originalEvent;
				if(iRate >= 1)
					return;
				if(ev.wheelDelta){
